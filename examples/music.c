...
int main(void)
{
    ...
    sl_stream_id music = sl_stream_load(RESOURCES_PATH "awesomeness.ogg");
    ...
    while (sl_frame_step())
    {
        ...
        if (!sl_stream_is_playing(music))
        {
            ...
            if (sl_key_just_pressed(SL_KEY_SPACE)) {
                sl_stream_play(music);
            }
        }
        else
        {
            sl_audio_set_volume_stream(sl_aduio_get_volume_stream() + sl_mouse_get_wheel().y * 0.05f);
            ...
            sl_render_set_color(SL_YELLOW);
            sl_render_text(sl_text_format("Volume: %i%%", (int)roundf(100 * sl_aduio_get_volume_stream())), SL_VEC2(10, 10), 24, SL_VEC2(2,2));

            if (sl_key_just_pressed(SL_KEY_SPACE)) {
                sl_stream_pause(music);
            }
            else if (sl_key_just_pressed(SL_KEY_S)) {
                sl_stream_stop(music);
            }
        }

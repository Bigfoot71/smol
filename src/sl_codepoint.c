...
int sl_codepoint_next(const char* text, int* codepointSize)
{
    const char* ptr = text;
...
int sl_codepoint_get_previous(const char* text, int* codepointSize)
{
    const char* ptr = text;
    int codepoint = 0x3f;       // Codepoint (defaults to '?')
    int cpSize = 0;
...
int sl_codepoint_from_utf8(const char* text, int* codepointSize)
{
    /*
...
int* sl_codepoint_from_utf8_str(const char* text, int* count)
{
    ...
    for (int i = 0; i < text_length; codepoint_count++) {
        codepoints[codepoint_count] = sl_codepoint_next(text + i, &codepointSize);
        i += codepointSize;
    }
